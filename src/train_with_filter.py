...
def main():
    ...
    for ep in range(1, args.episodes + 1):
        ...
        while not done and step < args.horizon:
            ...
            # Apply MPC safety filter BEFORE stepping the env
            u_prop = np.stack(actions, axis=0)           # (N, 2)
            #u_safe = mpc_filter(u_prop, env.pos.copy(), dt=env.dt, radius=env.radius)
            u_safe = mpc_filter(u_prop, env.pos.copy(), dt=env.dt, radius=env.radius, eps=0.1, lambda_slack=5000)
            action_vec = u_safe.reshape(-1).astype(np.float32)
            ...
            for i in range(args.n_agents):
                ...
                act_buf[i].append(u_prop[i].copy())  # store proposed (pre-filter) action for learning
        ...
        #print(f"[Filtered] Episode {ep:04d} | Return per agent {ep_return} | Collisions {ep_collisions} | Steps {step}")
